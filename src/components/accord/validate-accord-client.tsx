...
import { AlertTriangle, CheckCircle, ArrowLeft, Ban } from "lucide-react";
...
export function ValidateAccordClient({ token }: { token: string }) {
  ...
  async function handle(action: "accept" | "reject") {
    ...
    const res = await fetch(`/api/accords/${token}/valider`, {
      ...
      body: JSON.stringify({ 
        action, 
        commentaire: action === "reject" ? (commentaire || "Accord refusé sans commentaire additionnel.") : (commentaire || undefined) 
      }),
    });
    setLoading(false);
    if (res.ok) {
  ...
  return (
    <div className="mt-6">
      {!showRejectReason ? (
        /* Étape 1 : Choix principal - Focus sur l'acceptation (divulgation progressive) */
        ...
        /* Étape 2 : Confirmation de refus avec demande de motif (Sécurité cognitive) */
        <div className="page-enter space-y-5 rounded-xl border border-error-100 bg-error-50/30 p-5 md:p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-error-800">Confirmation de refus de l&apos;accord</h4>
              <p className="text-xs text-neutral-600 mt-1 leading-relaxed">
                Afin de documenter précisément la raison de votre refus auprès de l&apos;initiateur, veuillez expliquer ce qui ne convient pas dans les termes proposés.
              </p>
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="commentaire" className="text-xs font-bold uppercase tracking-wider text-neutral-600 block">
